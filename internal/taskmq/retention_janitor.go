...
func (j *RetentionJanitor) purgeCancelledDelayed(ctx context.Context) (int64, error) {
	delayedKey := KeysFor(j.queue).Delayed()
	...
	for {
		...
		for _, member := range members {
			task := &Task{}
			if err := j.codec.Unmarshal(unsafeStringToBytes(member), task); err != nil {
				continue
			}
			if task.ID == "" {
				continue
			...
			cancelledKey := KeysFor(q).Cancelled(task.ID)
