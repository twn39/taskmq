...
func RateLimitMiddleware(limiter *ratelimit.GCRALimiter, broker broker.TaskBroker, provider func(queue string) (int64, time.Duration, string), extractor func([]byte) string, cdc codec.Codec, logger *zap.Logger) CoreHandlerFunc {
	return func(c *ConsumeContext) error {
		...
		if c.Task.GroupKey != "" {
			...
			if inspector, ok := cdc.(codec.PayloadInspector); ok {
				if val, err := inspector.InspectField(c.Task.Payload, keyField); err == nil {
					groupKeyVal = val
				}
			} else {
				if val, err := codec.InspectJSONField(c.Task.Payload, keyField); err == nil {
					groupKeyVal = val
				}
			}
		}
...
func UniqueLockWatchdogMiddleware(broker broker.TaskBroker, logger *zap.Logger) CoreHandlerFunc {
	return func(c *ConsumeContext) error {
		task := c.Task
		if task.UniqueKey == "" {
			return c.Next()
		...
		// taskmodel.UniqueUntilStart: Release the lock immediately as execution starts
		if task.UniqueScope == taskmodel.UniqueUntilStart {
			if err := broker.ReleaseUniqueLock(c.Context, task); err != nil {
				logger.Error("Failed to release unique lock for taskmodel.UniqueUntilStart",
		...
		// taskmodel.UniqueUntilSuccess: If error occurs, release unique lock during retry delay
		if err != nil && task.UniqueScope == taskmodel.UniqueUntilSuccess {
			releaseCtx, cancelRelease := context.WithTimeout(context.Background(), 5*time.Second)
			rErr := broker.ReleaseUniqueLock(releaseCtx, task)
			cancelRelease()
			if rErr != nil {
				logger.Error("Failed to release unique lock on failure for taskmodel.UniqueUntilSuccess",
					zap.String("task_id", task.ID),
					zap.Error(rErr),
				)
			}
