...
func DefaultCronManagerFactory(rdb *redis.Client, logger *zap.Logger, queue string, c codec.Codec, healingInterval time.Duration, healingLockTTL time.Duration, scanBatchSize int, scanMaxCount int, lc *lifecycle.Lifecycle) CronManager {
...
func DefaultSchedulerFactory(rdb *redis.Client, logger *zap.Logger, queue string, cronManager CronManager, c codec.Codec, pollInterval time.Duration, streamHardLimit, streamMaxLen int64) Runner {
	return NewDelayedScheduler(rdb, logger, queue, cronManager, c, pollInterval, streamHardLimit, streamMaxLen)
}

// DefaultJanitorFactory constructs the standard PEL recovery janitor.
...
func DefaultRetentionJanitorFactory(rdb *redis.Client, logger *zap.Logger, queue string, group string, c codec.Codec, lc *lifecycle.Lifecycle) Runner {
