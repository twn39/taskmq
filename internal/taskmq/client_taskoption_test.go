...
func TestClient_TaskOptions(t *testing.T) {
	...
	t.Run("Option execution success - options correctly applied", func(t *testing.T) {
		task := NewTask("test:option", []byte("payload"))
		...
		err := client.Enqueue(context.Background(), task,
			...
			WithTaskUnique("my-unique-key", 30*time.Second, UniqueUntilStart),
		)
		...
		assert.Equal(t, UniqueUntilStart, task.UniqueScope)
	})

	t.Run("Option execution failure - validation error returned", func(t *testing.T) {
		task := NewTask("test:option:fail", []byte("payload"))
		...
		err = client.Enqueue(context.Background(), task, WithTaskUnique("", 10*time.Second, UniqueUntilSucceeded))
		assert.Error(t, err)
