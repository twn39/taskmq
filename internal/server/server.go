...
func NewServer(lc fx.Lifecycle, logger *zap.Logger, adminHandler *handler.AdminHandler, cfg *config.Config) *echo.Echo {
	...
	e.GET("/api/metrics/queues", adminHandler.GetQueueMetrics)
	e.GET("/api/metrics/queues/:queue", adminHandler.GetQueueMetrics)
	e.GET("/api/queues/:queue/tasks/:id", adminHandler.GetTask)
	e.POST("/api/queues/:queue/tasks/:id/progress", adminHandler.UpdateProgress)
	e.GET("/api/queues/:queue/events", adminHandler.ListEvents)
	e.GET("/api/queues/:queue/workers", adminHandler.ListWorkers)
	e.POST("/api/queues/:queue/bulk", adminHandler.BulkEnqueue)
